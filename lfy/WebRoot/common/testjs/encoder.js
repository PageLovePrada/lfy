Encoder = {
	EncodeType : "entity",
	isEmpty : function(a) {
		return a ? a === null || a.length == 0 || /^\s+$/.test(a) : true
	},
	...
	htmlDecode : function(e) {
		...
		arr = a.match(/&#[0-9]{1,5};/g);
		if (arr != null)
			for ( var d = 0; d < arr.length; d++) {
				b = arr[d];
				c = b.substring(2, b.length - 1);
				if (c >= -32768 && c <= 65535)
					a = a.replace(b, String.fromCharCode(c));
				else
					a = a.replace(b, "")
			}
		return a
	...
	hasEncoded : function(a) {
		return /&#[0-9]{1,5};/g.test(a) ? true
				: /&[A-Z]{2,6};/gi.test(a) ? true : false
	},
